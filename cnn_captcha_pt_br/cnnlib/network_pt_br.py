...
class CNN(object):
    def __init__(self, image_height, image_width, max_captcha, char_set, model_save_dir):
        ...
        self.model_save_dir = model_save_dir  # Caminho do modelo
        with tf.name_scope('parameters'):
            self.w_alpha = 0.01
            self.b_alpha = 0.1
        # tf espaço reservado para inicialização
        with tf.name_scope('data'):
            self.X = tf.placeholder(tf.float32, [None, self.image_height * self.image_width])  # Vetor de característica
            self.Y = tf.placeholder(tf.float32, [None, self.max_captcha * self.char_set_len])  # rótulo
            self.keep_prob = tf.placeholder(tf.float32)  # dropoutvalor

    @staticmethod
    def convert2gray(img):
        """
        A imagem é convertida para tons de cinza, se for uma imagem de 3 canais, ela será calculada, e a imagem de um canal será retornada diretamente
        :param img:
        :return:
        """
        if len(img.shape) > 2:
    ...
    def text2vec(self, text):
        """
        Converter tags para codificação oneHot
        :param text: str
        :return: numpy.array
        """
        text_len = len(text)
        if text_len > self.max_captcha:
            raise ValueError('O código de verificação pode ter até {} caracteres'.format(self.max_captcha))
